  ...
  return (
    <div className={`ticket-test-breakdown ${ticketBad ? "breakdown-failed" : "breakdown-passed"}`}>
      <div className="breakdown-header">
        <code className="breakdown-key">{ticket.ticket_key ?? "UNKNOWN"}</code>
        <span className={`breakdown-status ${ticketBad ? "status-fail" : "status-pass"}`}>
          {ticketBad ? "✗ fail" : "✓ pass"}
        </span>
        <div className="breakdown-counts">
          ...
          {breakdown.failedOutsideSelection.length > 0 ? (
            <span className="count-unknown">{breakdown.failedOutsideSelection.length} extra</span>
          ) : null}
          {noTests ? <span className="count-none">no tests ran</span> : null}
        </div>
      </div>
      ...
      {breakdown.selected.length > 0 ? (
        <ul className="test-case-list">
          {breakdown.selected.map((testName, index) => {
            ...
            const statusClass = failed ? "tc-failed" : passed ? "tc-passed" : "tc-unknown";
            const icon = failed ? "✗" : passed ? "✓" : "•";

            return (
              <li key={`${ticket.ticket_key ?? "unknown"}-${testName}-${index}`} className={`test-case-item ${statusClass}`}>
                <span className="tc-icon">{icon}</span>
                <span className="tc-name" title={testName}>{testName.split("::").pop() ?? testName}</span>
                <span className="tc-file">{testName.split("::")[0]}</span>
              </li>
            );
          })}
        </ul>
      ...
      {(noTests || breakdown.failedOutsideSelection.length > 0) && ticketBad && ticket.error ? (
        <p className="breakdown-error">{ticket.error.slice(0, 160)}</p>
      ) : null}
    </div>
  );
}
            ...
            <>
              ...
              <div className="report-grid-2x2">
                ...
                <article className="report-card validation-card">
                  ...
                  <div className="ticket-breakdown-list">
                    {(pipelineReport.tickets ?? []).map((ticket, idx) => (
                      <TicketTestBreakdown key={`${ticket.ticket_key ?? "unknown"}-${idx}`} ticket={ticket} />
                    ))}
                  </div>
                </article>
              </div>
